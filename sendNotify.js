    ...
    try {
        ...
        if (process.env.NOTIFY_AUTOCHECKCK == "true") {
            if (text.indexOf("已失效") != -1 || desp.indexOf("登录") != -1) {
                ...
                var strPtPin = await GetPtPin(text, desp);
        ...
        if (process.env["BARK_PUSH" + UseGroupNotify] && Use_BarkNotify) {
            ...
            if (process.env["BARK_GROUP" + UseGroupNotify]) {
                BARK_GROUP = process.env["BARK_GROUP" + UseGroupNotify];
            }
            if (process.env["BARK_ICON" + UseGroupNotify]) {
                BARK_ICON = process.env["BARK_ICON" + UseGroupNotify];
            }
        }
...
async function GetPtPin(text, desp) {
    try {
        let strPtPin = "";
        // 尝试从标题中提取（原逻辑）
        ...
        if (TempList.length > 1) {
            ...
            console.log(`从标题捕获别名: ${strNickName}`);
            if (TempCK) {
                for (let j = 0; j < TempCK.length; j++) {
                    if (TempCK[j].nickName === strNickName || TempCK[j].pt_pin === strNickName) {
                        strPtPin = TempCK[j].pt_pin;
                        console.log(`通过别名反查PtPin成功: ${strPtPin}`);
                        return strPtPin;
                    }
                }
        ...
        // 尝试从内容中提取 PtPin（新增逻辑）
        const pinMatch = desp.match(/账号\d+\s+([^\s]+)/);
        if (pinMatch) {
            strPtPin = pinMatch[1];
            console.log(`从内容中提取到PtPin: ${strPtPin}`);
            return strPtPin;
        }

        // 尝试匹配其他常见格式
        const jdPinMatch = text.match(/(jd_[^\s]+)/) || desp.match(/(jd_[^\s]+)/);
        if (jdPinMatch) {
            strPtPin = jdPinMatch[1];
            console.log(`通过正则匹配到PtPin: ${strPtPin}`);
            return strPtPin;
        }

        console.log(`无法从通知中提取PtPin`);
        return "";
    } catch (error) {
        console.error(`GetPtPin函数出错: ${error}`);
        return "";
    }
}
...
function BarkNotify(text, desp, params = {}) {
    return new Promise((resolve) => {
        if (BARK_PUSH) {
            ...
            // 如果设置了图标URL，添加到 JSON 对象中
            if (BARK_ICON) {
                jsonData.icon = BARK_ICON;
            }
            
            const options = {
                url: `${BARK_PUSH}`,
                json: jsonData, // 使用上面构建的 jsonData 对象
                headers: {
